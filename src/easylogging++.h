...
namespace easyloggingpp {
    ...
    public:
        ...
        static unsigned int convertFromString(const std::string &configStr) {
            ...
            if (configStr == "filename" || configStr == "FILENAME")
            ...
            if (configStr == "roll_out_size" || configStr == "ROLL_OUT_SIZE")
                return ConfigurationType::RollOutSize;
...
//!
            ...
            public:
                ...
                replaceAll(const std::string &str, const std::string &replaceWhat,
    ...
    static std::string getDeviceName(void) {
        ...
        return ss.str();
                ...
                getEnvironmentVariable(const char *variableName, const char *defaultVal,
                    ...
                    if ((val == NULL) || ((strcmp(val, "") == 0))) {
                        ...
                        std::string valBash = internal::utilities::OSUtils::getBashOutput(
                                alternativeBashCommand);
                        if (valBash.empty()) {
        ...
        currPath_ = STRTOK(currPath_, pathDelim_, &nextTok_);
            ...
            currPath_ = STRTOK(NULL, pathDelim_, &nextTok_);
                ...
                static std::string
                getPathFromFilename(const std::string &fullPath_, internal::Constants *constants_) {
                    if (fullPath_ == "" ||
                        fullPath_.find(constants_->PATH_SLASH) == std::string::npos) {
                        return fullPath_;
                    }
...
//!
            ...
            public:
                ...
                static void updateFormatValue(const std::string &formatSpecifier_,
                    ...
                    std::size_t foundAt = std::string::npos;
                    while ((foundAt = currentFormat_.find(formatSpecifier_, foundAt + 1)) !=
                           std::string::npos) {
                        if (currentFormat_[foundAt > 0 ? foundAt - 1 : 0] ==
                            constants_->FORMAT_SPECIFIER_ESCAPE_CHAR) {
                            currentFormat_.erase(foundAt > 0 ? foundAt - 1 : 0, 1);
                            ++foundAt;
                        } else {
                            currentFormat_ = currentFormat_.replace(foundAt,
                                                                    formatSpecifier_.size(),
                                                                    value_);
                            return;
                        }
                    }
...
#   endif // defined(_MSC_EXTENSIONS)
            ...
            tv->tv_usec = static_cast<long>(present_ % usecOffSet);
                ...
                static std::string getDateTime(const std::string &bufferFormat_, unsigned int type_,
                                               internal::Constants *constants_,
...
#if _ELPP_OS_UNIX
                    bool hasTime_ = ((type_ & constants_->kDateTime) ||
                                     (type_ & constants_->kTimeOnly));
                    timeval currTime;
        ...
        if ((type_ & constants_->kDateTime) || (type_ & constants_->kTimeOnly)) {
            if (GetTimeFormatA(LOCALE_USER_DEFAULT, 0, 0, kTimeFormatLocal_, dateBuffer_, kDateBuffSize_) != 0) {
                ...
                getTimeDifference(const timeval &endTime_, const timeval &startTime_) {
            ...
            inline void unregisterAll(void) {
                if (!empty()) {
                    std::for_each(list_.begin(), list_.end(),
                                  std::bind1st(std::mem_fun(&Registry::release), this));
                    list_.clear();
                }
            }
        ...
        public:
            ...
            public:
                ...
                bool operator()(const Configuration *conf_) {
                    return ((conf_ != NULL) && (conf_->level() == level_) &&
                            (conf_->type() == type_));
                }
...
//!
    class Configurations
            : public internal::Registry<internal::Configuration, internal::Configuration::Predicate> {
    public:
        //!
        //! Default constructor
        //!
        ...
        void set(unsigned int level_, unsigned int configurationType_, const std::string &value_,
            ...
            if ((configurationType_ == ConfigurationType::PerformanceTracking &&
                 level_ != Level::All) ||
                (configurationType_ == ConfigurationType::MillisecondsWidth &&
                 level_ != Level::All)) {
        ...
        bool parseFromFile(const std::string &configurationFile_, Configurations *base_ = NULL) {
            ...
            __EASYLOGGINGPP_ASSERT(fileStream_.is_open(),
                                   "Unable to open configuration file [" << configurationFile_
                                                                         << "] for parsing.");
            ...
            while (fileStream_.good()) {
                ...
                __EASYLOGGINGPP_ASSERT(parsedSuccessfully_,
...
#   endif // _ELPP_NDK
        ...
        //!        'All' may override configuration. Be careful with this.
        //!
        inline void setAll(unsigned int configurationType_, const std::string &value_,
                           bool skipLEVEL_ALL = false) {
            if (!skipLEVEL_ALL) {
        ...
        public:
            ...
            static bool
            parseLine(std::string &line, unsigned int &currLevel, Configurations *conf) {
                ...
                if (isConfig(line)) {
                    ...
                    currConfigStr = internal::utilities::StringUtils::stripAllWhiteSpaces(
                            currConfigStr);
...
//!
        ...
        public:
            TypedConfigurations(const Configurations &configurations,
                                internal::Constants *constants_) {
        ...
        private:
            ...
            void determineFormats(unsigned int level_, const std::string &originalFormat) {
                ...
                if (originalFormat.find(constants_->APP_NAME_FORMAT_SPECIFIER) !=
                    std::string::npos) {
                    formatSpec |= constants_->kAppName;
                }
                if (originalFormat.find(constants_->LOGGER_ID_FORMAT_SPECIFIER) !=
                    std::string::npos) {
                    formatSpec |= constants_->kLoggerId;
                }
                if (originalFormat.find(constants_->THREAD_ID_FORMAT_SPECIFIER) !=
                    std::string::npos) {
                    formatSpec |= constants_->kThreadId;
                }
                if (originalFormat.find(constants_->LOCATION_FORMAT_SPECIFIER) !=
                    std::string::npos) {
                    formatSpec |= constants_->kLocation;
                }
                if (originalFormat.find(constants_->FUNCTION_FORMAT_SPECIFIER) !=
                    std::string::npos) {
                ...
                if (originalFormat.find(constants_->LOG_MESSAGE_FORMAT_SPECIFIER) !=
                    std::string::npos) {
                    formatSpec |= constants_->kLogMessage;
                }
                if (originalFormat.find(constants_->VERBOSE_LEVEL_FORMAT_SPECIFIER) !=
                    std::string::npos) {
                    formatSpec |= constants_->kVerboseLevel;
                }
                if (originalFormat.find(constants_->DATE_TIME_FORMAT_SPECIFIER) !=
                    std::string::npos) {
                    formatSpec |= constants_->kDateTime;
                    setValue(level_, constants_->DATE_TIME_FORMAT_SPECIFIER,
                             dateFormatSpecifierMap_);
                } else if (originalFormat.find(constants_->DATE_ONLY_FORMAT_SPECIFIER) !=
                           std::string::npos) {
                    formatSpec |= constants_->kDateOnly;
                    setValue(level_, constants_->DATE_ONLY_FORMAT_SPECIFIER,
                             dateFormatSpecifierMap_);
                } else if (originalFormat.find(constants_->TIME_ONLY_FORMAT_SPECIFIER) !=
                           std::string::npos) {
                    formatSpec |= constants_->kTimeOnly;
                    setValue(level_, constants_->TIME_ONLY_FORMAT_SPECIFIER,
                             dateFormatSpecifierMap_);
                }
#if _ELPP_OS_UNIX
                ...
                switch (level_) {
                    case Level::Debug:
                        internal::utilities::LogManipulator::updateFormatValue(
                                constants_->LEVEL_FORMAT_SPECIFIER,
            ...
            void
            insertFilename(unsigned int level_, const std::string &fname_, bool forceNew = false) {
                ...
                std::string path_ = internal::utilities::OSUtils::getPathFromFilename(fnameFull,
                                                                                      constants_);
            ...
            unsigned long getULong(const std::string &confValue_) {
                ...
                if (trimmedVal.size() == 0) {
                    ...
                    __EASYLOGGINGPP_ASSERT(valid, "Configuration value not a valid integer "
                            << trimmedVal);
                }
                ...
                __EASYLOGGINGPP_SUPPRESS_UNSED(valid);
                __EASYLOGGINGPP_ASSERT(valid,
                                       "Configuration value not a valid integer " << trimmedVal);
            ...
            bool
            checkRollOuts(unsigned int level_, unsigned int &validLevel_, std::string &fname_) {
                std::fstream *fs = fileStream(level_);
            ...
            template<typename T>
            inline unsigned int
            findValidLevel(internal::ConfigurationMap<T> &map_, unsigned int refLevel_) {
...
//!
        class RegisteredCounters
                : public Registry<internal::LogCounter, internal::LogCounter::Predicate> {
        public:
            bool validate(const char *file_, unsigned long int line_, std::size_t n_,
                          internal::Constants *constants_) {
#if _ELPP_ENABLE_MUTEX
                                                                                                                                        internal::threading::ScopedLock slock_(mutex_);
        __EASYLOGGINGPP_SUPPRESS_UNSED(slock_);
...
//!
        ...
        public:
            ...
            inline bool
            validateCounter(const char *file_, unsigned long int line_, std::size_t n_) {
        ...
        __EASYLOGGINGPP_SUPPRESS_UNSED(slock_);
            ...
            void setApplicationArguments(int argc, char **argv) {
                while (argc-- > 0) {
                    ...
                    if ((strlen(argv[argc]) >= 5) && (argv[argc][0] == '-') &&
                        (argv[argc][1] == '-') &&
                        (argv[argc][2] == 'v') && (argv[argc][3] == '=') &&
                        (isdigit(argv[argc][4]))) {
                        ...
                        // Look for -v argument
                    else if ((strlen(argv[argc]) == 2) && (argv[argc][0] == '-') &&
                             (argv[argc][1] == 'v')) {
                        constants_->CURRENT_VERBOSE_LEVEL = constants_->MAX_VERBOSE_LEVEL;
                    }
                        // Look for --verbose argument
                    else if ((strlen(argv[argc]) == 9) && (argv[argc][0] == '-') &&
                             (argv[argc][1] == '-') &&
                             (argv[argc][2] == 'v') && (argv[argc][3] == 'e') &&
                             (argv[argc][4] == 'r') &&
                             (argv[argc][5] == 'b') && (argv[argc][6] == 'o') &&
                             (argv[argc][7] == 's') &&
                             (argv[argc][8] == 'e')) {
                        constants_->CURRENT_VERBOSE_LEVEL = constants_->MAX_VERBOSE_LEVEL;
                    }
...
#define _ELPP_STREAM(l) (*(l->stream()))
        ...
        public:
            Writer(const std::string &loggerId_,
                ...
                if (logger_ == NULL) {
                    __EASYLOGGINGPP_ASSERT(logger_ != NULL, "Logger [" << loggerId_
                                                                       << "] not registered or configured yet!");
                    proceed_ = false;
                }
#if _ELPP_ENABLE_MUTEX
...
#   endif // (!_ELPP_QT_5)
    ...
    inline Writer& operator<<(const QStack<T>& stack_) {
        ...
        private:
            ...
            inline Writer &writeIterator(Iterator begin_, Iterator end_, std::size_t size_) {
                _ELPP_STREAM(logger_) << "[";
                for (std::size_t i = 0;
                     begin_ != end_ && i < constants_->MAX_LOG_PER_CONTAINER; ++i, ++begin_) {
                    operator<<(*begin_);
            ...
            void buildAndWriteLine(void) {
                ...
                if (f_ & constants_->kAppName) {
                    ...
                    internal::utilities::LogManipulator::updateFormatValue(fs_, v_, currLine_,
                                                                           constants_);
                }
                // Logger ID
                ...
                if (f_ & constants_->kThreadId) {
                    ...
                    internal::utilities::LogManipulator::updateFormatValue(fs_, ss.str(), currLine_,
                                                                           constants_);
                }
                // Date/Time
                if ((f_ & constants_->kDateOnly) || (f_ & constants_->kTimeOnly) ||
                    (f_ & constants_->kDateTime)) {
                    v_ = internal::utilities::DateUtils::getDateTime(dateFormat,
                                                                     f_, constants_,
                                                                     conf_->millisecondsWidth(
                                                                             Level::All));
                ...
                if (f_ & constants_->kLocation) {
                    ...
                    internal::utilities::LogManipulator::updateFormatValue(fs_, tempss_.str(),
                                                                           currLine_, constants_);
                    tempss_.str("");
                }
                // User
                ...
                if (f_ & constants_->kLogMessage) {
                    ...
                    internal::utilities::LogManipulator::updateFormatValue(fs_,
                                                                           logger_->stream()->str(),
                                                                           currLine_, constants_);
                }
                log();
        ...
        return false;
            ...
            inline void syncWritePointer(unsigned int level_, Logger *targetLogger_,
                                         std::fstream *baseStream_) {
                targetLogger_->acquireLock();
                targetLogger_->typedConfigurations_->fileStream(level_)->seekg(
                        baseStream_->tellg());
    ...
    public:
        ...
        static inline const std::string releaseDate(void) {
            return std::string("12-07-2013 1243hrs");
        }

        // Original author and maintainer
        static inline const std::string author(void) {
            return std::string("Majid Khan <mkhan3189@gmail.com>");
        }

        // Web link
        static inline const std::string website(void) {
            return std::string("http://icplusplus.com/tools/easylogging");
        }
        ...
        static inline const std::string copyright(void) {
            return std::string("Copyright (c) 2012 - 2013 Majid Khan");
        }

        // Full licence
        static const std::string licence(void) {
            std::stringstream ss;
            ss << "   This software is provided 'as-is', without any express or implied"
               << std::endl;
            ss << "   warranty. In no event will the authors be held liable for any damages"
               << std::endl;
            ss << "   arising from the use of this software." << std::endl;
            ss << std::endl;
            ss << "   Permission is granted to anyone to use this software for any purpose,"
               << std::endl;
            ss << "   including commercial applications, and to alter it and redistribute"
               << std::endl;
            ss << "   it freely, subject to the following restrictions:" << std::endl;
            ss << std::endl;
            ss << "   1. The origin of this software must not be misrepresented; you must"
               << std::endl;
            ss << "      not claim that you wrote the original software. If you use this"
               << std::endl;
            ss << "      software in a product, an acknowledgment in the product documentation"
               << std::endl;
            ss << "      would be appreciated but is not required." << std::endl;
            ss << std::endl;
            ss << "   2. Altered source versions must be plainly marked as such, and must"
               << std::endl;
...
//!
    ...
    public:
        ...
        reconfigureLogger(Logger *logger_, const Configurations &configurations_) {
        ...
        //!
        static inline Logger *
        reconfigureLogger(const std::string &identifier_, Configurations &configurations_) {
        ...
        //!
        static inline void
        reconfigureAllLoggers(unsigned int configurationType_, const std::string &value_) {
            for (std::size_t i = 0; i < internal::registeredLoggers->count(); ++i) {
                Logger *l = internal::registeredLoggers->at(i);
        ...
        //!
        static inline void setDefaultConfigurations(Configurations &configurations,
                                                    bool configureExistingLoggers = false) {
        ...
        public:
            ...
            static inline bool
            enabled(internal::TypedConfigurations *conf_, unsigned int level_ = Level::All) {
                __EASYLOGGINGPP_ASSERT(conf_ != NULL,
                                       "Invalid TypedConfigurations provided - nullptr");
            ...
            static inline bool
            toFile(internal::TypedConfigurations *conf_, unsigned int level_ = Level::All) {
                __EASYLOGGINGPP_ASSERT(conf_ != NULL,
                                       "Invalid TypedConfigurations provided - nullptr");
            ...
            filename(Logger *logger_, unsigned int level_ = Level::All) {
                __EASYLOGGINGPP_ASSERT(logger_ != NULL, "Invalid Logger provided - nullptr");
            ...
            static inline bool toStandardOutput(internal::TypedConfigurations *conf_,
                                                unsigned int level_ = Level::All) {
                __EASYLOGGINGPP_ASSERT(conf_ != NULL,
                                       "Invalid TypedConfigurations provided - nullptr");
            ...
            static inline const std::string &
            logFormat(Logger *logger_, unsigned int level_ = Level::All) {
                __EASYLOGGINGPP_ASSERT(logger_ != NULL, "Invalid Logger provided - nullptr");
            ...
            static inline int millisecondsWidth(internal::TypedConfigurations *conf_,
                                                unsigned int level_ = Level::All) {
                __EASYLOGGINGPP_ASSERT(conf_ != NULL,
                                       "Invalid TypedConfigurations provided - nullptr");
            ...
            static inline bool
            performanceTracking(Logger *logger_, unsigned int level_ = Level::All) {
                __EASYLOGGINGPP_ASSERT(logger_ != NULL, "Invalid Logger provided - nullptr");
                return constConf(logger_)->performanceTracking(level_);
            }

            static inline bool performanceTracking(internal::TypedConfigurations *conf_,
                                                   unsigned int level_ = Level::All) {
                __EASYLOGGINGPP_ASSERT(conf_ != NULL,
                                       "Invalid TypedConfigurations provided - nullptr");
            ...
            logRollOutSize(Logger *logger_, unsigned int level_ = Level::All) {
                __EASYLOGGINGPP_ASSERT(logger_ != NULL, "Invalid Logger provided - nullptr");
